...
#Second Block
    def __init__(
        self,
        vision_config=None,
        text_config=None,   # Configiration of decoder
        ignore_index=-100,  # During training, image tokens are not counted for loss, ignore image tokens
        ...
        **kwargs,
        ...
        self.vision_config.projecion_dim = projection_dim
...
def repeat_kv(hidden_states: torch.Tensor, n_rep: int) -> torch.Tensor:
    ...
    hidden_states = hidden_states[:, :, None, :, :].expand(batch, num_key_value_heads, n_rep, slen, head_dim) # Insert a new dimension at None, then expand 
...
# The tenth block
    ...
    def forward(self, x, position_ids, seq_len=None):
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            ...
            emb = torch.cat((freqs, freqs), dim=-1)   # Different from the implementation of the original paper
...
def apply_rotary_pos_embed(q, k, cos, sin, unsqueeze_dim=1):
    ...
    cos = cos.unsqueeze(unsqueeze_dim) # Add the head dimension [Batch_Size,1(will be broad cast to num_heads), Seq_Len, Head_Dim]
...
#Eighth Block
    def __init__(self, config: GemmaConfig, layer_idx: Optional[int] = None):
        ...
        # Example
        # num_heads(query heads) = 8, hidden_size = 1024, head_dim = hidden_size / num_heads = 128
        # num_key_value_heads = 2
        # Wq = [1024, 8*128 = 1024]
        # Wk = [1024, 2*128 = 256]
        # Wv = [1024, 2*128 = 256]
        # Every four query heads share the same key and value head
    ...
    def forward(
        ...
        **kwargs,
        ...
        assert attention_mask is not None   ### PaliGemma does not require casual mask during prefill
...
class GemmaForCausalLM(nn.Module):    ### A transformer model plus linear head to convert embeds into logits
#Fourth Block
    def __init__(self, config: GemmaConfig):
        super().__init__()
        self.config = config
        self.model = GemmaModel(config)   # A transformer model
        self.vocab_size = config.vocab_size
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=True) # The linear converter
...
#Third Blaock
    def __init__(self, config: PaliGemmaConfig):
        super().__init__()
        self.projector = nn.Linear(config.vision_config.hidden_size, config.projection_dim, bias=True)
...
#First Block
    ...
    def _merge_input_ids_with_image_features(
        ...
        if kv_cache is None or kv_cache.num_items() == 0:
            ...
            ) # fill_value=0: PaliGemma lm is designed in a way that all the image tokens 
              # and prefix tokens can attend to each other to bettter understand the task
        else:
        ...
        # [Batch_Size, Q_Len, KV_Len] -> [Batch_Size, Num_Heads_Q, Q_Len, KV_Len], here Num_Heads_Q is equal to 1
